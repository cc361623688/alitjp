...
function(e, t) {
	...
	s.anim = ["layer-anim", "layer-anim-01", "layer-anim-02", "layer-anim-03", "layer-anim-04", "layer-anim-05", "layer-anim-06"], l.pt.config = {
		...
		var n = this,
			...
			d = r.title ? '<div class="layui-layer-title '+(r.tjpcms_clstitle||'')+'" id= '+(r.tjpcms_titleid||'')+' style="' + (f ? r.title[1] : "") + '">' + (f ? r.title[0] : r.title) + "</div>" : "";
		return r.zIndex = l, t([r.shade ? '<div class="layui-layer-shade" id="layui-layer-shade' + a + '" times="' + a + '" style="' + ("z-index:" + (l - 1) + "; background-color:" + (r.shade[1] || "#000") + "; opacity:" + (r.shade[0] || r.shade) + "; filter:alpha(opacity=" + (100 * r.shade[0] || 100 * r.shade) + ");") + '"></div>' : "", '<div  class="' + s[0] +' '+(r.tjpcms_clsouter||'')+ ( " layui-layer-" + o.type[r.type]) + (0 != r.type && 2 != r.type || r.shade ? "" : " layui-layer-border") + " " + (r.skin || "") + '" id="' + s[0] + a + '" type="' + o.type[r.type] + '" times="' + a + '" showtime="' + r.time + '" conType="' + (e ? "object" : "string") + '" style="z-index: ' + l + "; width:" + r.area[0] + ";height:" + r.area[1] + (r.fixed ? "" : ";position:absolute;") + '">' + (e && 2 != r.type ? "" : d) + '<div id="' + (r.id || "") + '" class="layui-layer-content'+' '+(r.tjpcms_clscont||'') + (0 == r.type && r.icon !== -1 ? " layui-layer-padding" : "") + (3 == r.type ? " layui-layer-loading" + r.icon : "") + '">' + (0 == r.type && r.icon !== -1 ? '<i class="layui-layer-ico layui-layer-ico' + r.icon + '"></i>' : "") + (1 == r.type && e ? "" : r.content || "") + '</div><span class="layui-layer-setwin">' +
		function() {
		...
		t.tips[1] || f.remove(), l.autoLeft = function() {
			...
			left: l.tipLeft - (t.fixed ? n.scrollLeft() : 0),//这里我遇到个BUG，捐赠作者这里弹出图片，图片右侧没有全部显示出来，导致底部有水平滚动条了，解决了，是图片还未加载就计算了弹出层宽度，导致计算异常
